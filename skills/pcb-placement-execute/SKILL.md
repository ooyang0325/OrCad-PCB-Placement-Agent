...
description: Autonomously execute an exact reviewed PCB placement proposal and visually inspect the native result; never silently save.
...
proposals. Revision saves remain disabled unless the operator explicitly enables
interactive writes. Do not change client/server configuration yourself.
