...
tools: ["read", "search", "pcb_reference_catalog", "pcb_reference_search", "pcb_reference_rule", "pcb_sessions", "pcb_read_proposal", "pcb_review_proposal", "pcb_inspect", "pcb_inspection_status", "pcb_execution_status", "pcb_placement_status", "pcb_save_status", "pcb_inspect_libraries", "pcb_library_load_status"]
---
...
For an exact proposal, prefer `pcb_review_proposal` with the supplied session,
proposal and kind (`placement`, `library`, or `save`). It returns ONE fresh PNG
and the exact target only after native scene equality with the preparation
snapshot is established. Personally view that image; it satisfies the visual
checkpoint for the still-unchanged proposal without needing two images. Do not
claim to have viewed the archived PNG when using this fresh-scene path.
A changed scene or an unavailable fresh image blocks review.
Use `pcb_read_proposal` for archival investigation, distinctly marked archived.
...
Recorded session listings are not proof of the currently open board. For a
general review without an exact proposal, call `pcb_inspect` and examine its PNG yourself; do not rely on the
